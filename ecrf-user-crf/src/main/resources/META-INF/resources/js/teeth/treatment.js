...
function checkIfCanEnableAddButton() {
	const _addBtn = document.getElementById('addTreatmentBtn');
	const _dateInput = document.getElementById('treatmentDate');

	if (!_addBtn || !_dateInput) return;

	const _statusRadios = Array.from(
		document.querySelectorAll('input[name="status"]')
	);
	const _permanentCRadios = Array.from(
		document.querySelectorAll('input[name="permanentC"]')
	);
	const _permanentDDRadios = Array.from(
		document.querySelectorAll('input[name="permanentDD"]')
	);

	if (!(_statusRadios && _permanentCRadios && _permanentDDRadios)) {
		_addBtn.disabled = false;
		return;
	}

	console.log('check add enable');
	// each input null check
	const isStatusSelected = [..._statusRadios].some((input) =>
		input ? input.checked : false
	);
	const isPermanentCSelected = [..._permanentCRadios].some((input) =>
		input ? input.checked : false
	);
	const isPermanentDDSelected = [..._permanentDDRadios].some((input) =>
		input ? input.checked : false
	);
	const isDateSelected = _dateInput.value !== '';

	if (
		(isStatusSelected || isPermanentCSelected || isPermanentDDSelected) &&
		isDateSelected
	) {
		_addBtn.disabled = false;
	} else {
...
document.addEventListener('DOMContentLoaded', () => {
	...
	const passedTeethNames = teethsParam
		.split(',')
		.map((num) =>
			num.trim().startsWith('Teeth') ? num.trim() : 'Teeth' + num.trim()
		)
	...
	const statusRadios = Array.from(
		document.querySelectorAll('input[name="status"]')
	);
	const permanentCRadios = Array.from(
		document.querySelectorAll('input[name="permanentC"]')
	);
	const permanentDDRadios = Array.from(
		document.querySelectorAll('input[name="permanentDD"]')
	);
	...
	//이벤트 연결
	[...statusRadios, ...permanentCRadios, ...permanentDDRadios].forEach(
		(radio) => {
	...
	addBtn.addEventListener('click', () => {
		...
		const selectedStateC =
			(permanentCRadios.find((r) => r.checked) || {}).value || '';

		const selectedStateDDArr = Array.from(
			document.querySelectorAll('input[name="permanentDD"]:checked')
		).map((cb) => cb.value);

		let treatmentDate = treatmentDateInput.value
			? new Date(treatmentDateInput.value)
			: new Date();

		//C랑 D에서 받은거 합치기
		const selectedStateArr = [selectedStateC, ...selectedStateDDArr].filter(
			(val) => val
		);
		...
		if (teethsParam) {
			...
			const treatmentDateStr = treatmentDateObj
				.toISOString()
				.split('T')[0];
			...
			if (tbody) {
				...
				rows.forEach((row) => {
					...
					if (
						dateCell &&
						dateCell.textContent.trim() === treatmentDateStr
					) {
			...
			console.log(
				`중복 제거 완료: 날짜 ${treatmentDateStr} 에 해당하는 기존 항목 삭제`
			);

			addTeethListPendingTreatment(
				teethsParam,
				treatmentDate,
				selectedTreatment,
				selectedState
		...
		if (newlyAddedTeeth.length) {
			...
			tr.querySelector('.clear-btn').addEventListener('click', (e) => {
				...
				document
					.querySelectorAll('#treatmentTable tbody tr')
					.forEach((row) => {
		...
		function addTeethListPendingTreatment(
			teethsParam,
			treatmentDate,
			treatment,
			state
	...
	addDBBtn.addEventListener('click', () => {
		...
		$.ajax({
			...
			data: JSON.stringify({
				...
				crfId: commonData.crfId,
				subjectId: commonData.subjectId,
				linkId: commonData.linkId,
			}),
	...
	function groupAndCombineTreatments(pendingList) {
		...
		return Object.values(grouped).map((g) => {
			...
			params.set(
				'selectedTreatment',
				[...g.statuses].join(',').replace(/\s+/g, '') || '-'
			);
			params.set('selectedTeeth', g.selectedTeeth);
			params.set(
				'selectedState',
				[...g.permanents].join(',').replace(/\s+/g, '') || '-'
			);
	...
	treatmentDateInput.addEventListener('change', function () {
		...
		// 생후 연월 계산
		const birth = window.teeth.birth;
		...
		const matched = allHistoryData.filter(
			(item) => item.date === dateStr && regions.includes(item.region)
		);
		const matchedPending = pendingTreatments.filter(
			(item) => item.treatmentDate === dateStr
		);
		...
		*/
		...
		matchedPending.forEach((item) => {
			...
			document
				.querySelectorAll('input[name="permanentC"]')
				.forEach((input) => {
					input.checked = false;
				});

			document
				.querySelectorAll('input[name="permanentDD"]')
				.forEach((input) => {
					input.checked = false;
				});
			...
			if (item.selectedState) {
				item.selectedState.split(',').forEach((s) => {
					...
					const elC = document.querySelector(
						'input[name="permanentC"][value="' + v + '"]'
					);
					if (elC) {
						elC.checked = true;
					...
					const elDD = document.querySelector(
						'input[name="permanentDD"][value="' + v + '"]'
					);
					if (elDD) {
						elDD.checked = true;
		...
		const disablePermanentCGroup = permanentCValues.some((v) =>
			usedPermanents.has(v)
		);
		document
			.querySelectorAll('input[name="permanentC"]')
			.forEach((radio) => {
