...
@Component(
	    property = {
	        ...
	        "mvc.command.name="+ ECRFUserMVCCommand.RESOURCE_EDIT_TREATMENT
...
public class editTeethTreatmentResourceCommand extends BaseMVCResourceCommand  {
	...
	protected void doServeResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse) throws Exception {
		...
		TreatmentHistory past = null;
		
		if(treatmentId > 0) {
			past = _treatmentHistoryLocalService.getTreatmentHistory(treatmentId);
		}

		// get param
		String state = ParamUtil.getString(resourceRequest, "state", "");
		String treatment = ParamUtil.getString(resourceRequest, "treatment", "");

		resourceResponse.setContentType("application/json");
		PrintWriter writer = resourceResponse.getWriter();
		JSONObject rpObj = JSONFactoryUtil.createJSONObject();
		
		_log.info(String.valueOf(treatmentId) + " / " + state + " / " + treatment);

		ThemeDisplay themeDisplay = (ThemeDisplay)resourceRequest.getAttribute(WebKeys.THEME_DISPLAY);
		long groupId = themeDisplay.getScopeGroupId();
		User user = themeDisplay.getUser();
		ServiceContext treatmentServiceContext = ServiceContextFactory.getInstance(TreatmentHistory.class.getName(), resourceRequest);
		ServiceContext auditServiceContext = ServiceContextFactory.getInstance(TreatmentAudit.class.getName(), resourceRequest);

		long crfId = 0;
		long subjectId = 0;
		long linkId = 0;

		if(Validator.isNotNull(past)) {
			subjectId = past.getPatientID();
			crfId = past.getCrfId();
			linkId = past.getLinkId();
		}
		...
		// update treatment and add audit
		if(treatmentId > 0) {
			Date editedDate = new Date();
			
			// update treatment
			TreatmentHistory updated = _treatmentHistoryLocalService.UpdateHistory(
				treatmentId,
				treatment,
				state,
				editedDate,
				user.getUserId(),
				treatmentServiceContext
			);
			...
			TreatmentAudit audit = _treatmentAuditLocalService.AddAudit(
				...
				editedDate,
				"Update",
				past.getState() + " / " + past.getTreatment(),
				updated.getState() + " / " + updated.getTreatment(),
				auditServiceContext
			);
		}
